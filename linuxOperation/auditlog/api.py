...
def api_create_admin_log(request, instance, extend_type=None, extend_content=None, action=1, is_extend=True, changes=None):
    ...
    if action not in dict(LogEntry.Action.choices):
        raise TypeError(u" extend_type not in AUDITLOG_EXTEND_TYPE or AUDITLOG_EXTEND_TYPE not in settings;"
                        u"data like AUDITLOG_EXTEND_TYPE = ( ('login', _(u'登录')), ('logout', _(u'登出')),). ")
    if not ( changes and isinstance(changes, dict) ):
        changes = {}
    ...
    LogEntry.objects.create(
        ...
        additional_data=additional_data, domain_id=request.session.get('domain_id', 0),
        is_extend=is_extend, extend_type=extend_type, extend_content=extend_content
    )
    return True
