...
# 类型 必须为英文，否则不能进行搜索， 比如 'login'
_AUDITLOG_EXTEND_TYPE = (
    ('login', _(u"登录")),
    ('logout', _(u"登出")),
    ('coreconfig', _(u"系统设置")),
    ('domainconfig', _(u"域名设置")),
    ('reviewcondition', _(u"审核条件")),
    ('cfiltercond', _(u"内容过滤器条件")),
    ('cfilteraction', _(u"内容过滤器动作")),
)
AUDITLOG_EXTEND_TYPE = getattr(settings, 'AUDITLOG_EXTEND_TYPE', _AUDITLOG_EXTEND_TYPE)
...
class LogEntryManager(models.Manager):
    ...
    def log_create(self, instance, **kwargs):
        ...
        if changes is not None:
            ...
            kwargs.setdefault('content_type', ctype)
...
class LogEntry(models.Model):
    ...
    domain_id = models.IntegerField(_(u'所属域名ID'), default=0, db_index=True)
    is_extend = models.BooleanField(_(u'是否是扩展的类型'), default=0, help_text=u"api使用")
    extend_type = models.CharField(_(u'扩展类型'), choices=AUDITLOG_EXTEND_TYPE, null=True, max_length=50, db_index=True, blank=True, help_text=u"api使用")
    extend_content = models.TextField(_(u'扩展说明'), null=True, blank=True, help_text=u"api使用")
...
class AuditlogContentype(models.Model):
    content_type = models.OneToOneField(to='contenttypes.ContentType', on_delete=models.CASCADE, related_name='+',
                                        verbose_name=_("content type"))
    class Meta:
        db_table = 'auditlog_auditlogcontentype'
        managed = False
        verbose_name = _(u"统计入库此处的content_type_id")
        verbose_name_plural = _(u"统计入库此处的content_type_id")
