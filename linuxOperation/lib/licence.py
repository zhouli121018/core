...
class Licence(object):
    ...
    def __init__(self, licence_data='', licence_file=LICENCE_DEFAULT_FILE):
        ...
        if licence_data:
            ...
            self.licence_data = open(licence_file, 'r').read() if os.path.exists(licence_file) else ''
    ...
    def _parse_info_1_2(self, raw_info):
        if isinstance(raw_info['extra_module'], dict):
            extra_module = {}
            for k,v in raw_info['extra_module'].items():
                if not v:
                    continue
                extra_module[k] = v
        else:
...
if __name__ == "__main__":
    ...
    licence = Licence(licence_file=filepath)
    info = licence.licence_info
    if info and info['evaluation']:
        from app.core.models import DomainAttr
        value = DomainAttr.getAttrObjValue(domain_id=1,type='system',item='created')
        if value:
            start = time.mktime(time.strptime(value,'%Y-%m-%d %H:%M:%S'))
