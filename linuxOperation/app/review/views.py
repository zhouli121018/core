...
def ajax_reviewrule_list(request):
    ...
    colums = ['id', 'id', 'name', 'review__id', 'workmode', 'pre_action', 'cond_logic', 'cond_logic', 'sequence', 'disabled']
...
@csrf_exempt
@licence_required
def ajax_reviewrule_cond_add(request):
    data = request.POST
    try:
    ...
    if rule_id > 0:
        instance = ReviewRule.objects.get(pk=rule_id)
        form = ReviewRuleForm(instance=instance,post=data,request=request)
    else:
        form = ReviewRuleForm(post=data,request=request)
    if form.is_valid():
        form.save()
        response = {u"result":1}
    else:
        response = {u"result":0, u"failure":form.fail_resaon()}
