...
class MailboxStatForm(forms.Form):
    username = forms.CharField(label=u'帐号名', max_length=100, required=False, widget=forms.TextInput(attrs={"id":"id_username",'size':12, 'placeholder': u""}))
    name = forms.CharField(label=u'姓名', max_length=100, required=False, widget=forms.TextInput(attrs={"id":"id_name",'size':12, 'placeholder': u""}))
    ...
    send_permit = forms.ChoiceField(label=u'发送权限', required=False, choices=MAILLOG_SEND_PERMIT, widget=forms.Select(attrs={"id":"id_send_permit",'size':1}))
    recv_permit = forms.ChoiceField(label=u'接收权限', required=False, choices=MAILLOG_RECV_PERMIT, widget=forms.Select(attrs={"id":"id_recv_permit",'size':1}))
    ...
    disabled = forms.ChoiceField(label=u'邮箱状态', required=False, choices=MAILLOG_MAILBOX_STATUS, widget=forms.Select(attrs={"id":"id_disabled",'size':1}))
...
class ActiveUserStatForm(forms.Form):
    username = forms.CharField(label=u'帐号名', max_length=100, required=False,
                               widget=forms.TextInput(attrs={"id":"id_username",'size':12, 'placeholder': u""}))
    department = forms.ChoiceField(label=u'部门', required=False, choices=get_department_list(),
                                   widget=forms.Select(attrs={"id": "id_department", 'placeholder': u""}))
    date_select = forms.ChoiceField(label=u'开始日期', required=False,
                                    widget=forms.Select(attrs={"id": "id_date_select", 'placeholder': u""}))
    date_select2 = forms.ChoiceField(label=u'结束日期', required=False,
...
class MailLogSearchForm(forms.Form):
    type = forms.ChoiceField(label=u'发送类型', choices=( (u"",u"所有"),(u"in",u"收信"),(u"out",u"发信"),), required=False, initial="")
    username = forms.CharField(label=u'用户名', max_length=100, required=False,
                               widget=forms.TextInput(attrs={"id": "id_username", 'size': 12, 'placeholder': u""}))
    max_attach = forms.FloatField(label=u'最大附件(MB)', required=False, widget=forms.NumberInput(
        attrs={"id": "id_max_attach", 'size': 12, 'placeholder': u""}))
    min_attach = forms.FloatField(label=u'最小附件(MB)', required=False, widget=forms.NumberInput(
        attrs={"id": "id_min_attach", 'size': 12, 'placeholder': u""}))
    send_mail = forms.CharField(label=u'发信人', max_length=100, required=False,
                                widget=forms.TextInput(attrs={"id": "id_send_mail", 'size': 12, 'placeholder': u""}))
    recv_mail = forms.CharField(label=u'收信人', max_length=100, required=False,
                                widget=forms.TextInput(attrs={"id": "id_recv_mail", 'size': 12, 'placeholder': u""}))
    senderip = forms.CharField(label=u'发信服务器', max_length=100, required=False,
                               widget=forms.TextInput(attrs={'size': 12, 'placeholder': u""}))
    rcv_server = forms.CharField(label=u'收件服务器', max_length=100, required=False,
                                 widget=forms.TextInput(attrs={'size': 12, 'placeholder': u""}))
    ...
    result = forms.ChoiceField(label=u'投递结果', required=False, choices=MAILLOG_SEARCH_RESULT,
                               widget=forms.Select(attrs={"id": "id_log_result", 'placeholder': u""}))
    text = forms.CharField(label=u'关键字查询', max_length=200, required=False, widget=forms.TextInput(attrs={'size':12, 'placeholder': u'标题/发件邮箱/收件邮箱/附件名称'}))
...
class UserLogForm(forms.Form):
    start_time = forms.DateTimeField(label=u'开始时间', required=False)
    end_time = forms.DateTimeField(label=u'结束时间', required=False)
    username = forms.CharField(label=u'用户名', max_length=100, required=False)
    ip = forms.CharField(label=u'IP', max_length=20, required=False)
    classify = forms.ChoiceField(label=u'操作类型', choices=LOG_CLASSIFY, required=False, initial="")
    result = forms.ChoiceField(label=u'结果', choices=( ("", u'----'),  ("1", u'成功'), ("-1", u'失败'),), required=False, initial="")
...
class VisitLogForm(forms.Form):
    name = forms.CharField(label=u'用户名', max_length=100, required=False)
    username = forms.CharField(label=u'邮箱地址', max_length=100, required=False)
    start_time = forms.DateTimeField(label=u'登录开始时间', required=False)
    end_time = forms.DateTimeField(label=u'登录结束时间', required=False)
    online_time_gt = forms.IntegerField(label=u'在线时长(大于)', required=False)
    online_time_lt = forms.IntegerField(label=u'在线时长(小于)',required=False)
    ip = forms.CharField(label=u'IP', max_length=20, required=False)
    is_online = forms.ChoiceField(label=u'是否在线', choices=( ("", u'----'),  ("1", u'在线'), ("-1", u'离线'),), required=False, initial="")
    login_type = forms.CharField(label=u'登录方式', max_length=20, required=False)
...
class AuthLogForm(forms.Form):
    username = forms.CharField(label=u'邮箱地址', max_length=100, required=False)
    vtype = forms.ChoiceField(label=u'访问类型', choices=(("", u'----'), ("imap", 'imap'), ("pop3", "pop3"), ("smtp", "smtp") ),
                              required=False, initial="")
    start_time = forms.DateTimeField(label=u'访问开始时间', required=False)
    end_time = forms.DateTimeField(label=u'访问结束时间', required=False)
    ip = forms.CharField(label=u'IP', max_length=20, required=False)
    is_login = forms.ChoiceField(label=u'状态', choices=(("", u'----'), ("1", _(u'是')), ("-1", _(u'否'))), required=False,
                                 initial="")
