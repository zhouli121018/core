...
def choose_mailbox_list(request):
    ...
    返回值
    {
        ...
        dept:[
            {"id":x, "name":y, "parent":z, "child":"1"} 若无子部门则 child 不存在
        ]
    }
    """
    def get_dept_list( lists_dpt, current_dpt_id=0 ):
        dataDept = {}
    ...
    if action == "all":
        ...
        data["dept"] = get_dept_list( lists_dpt )
    elif action == "dept_maintan":
        current_dpt_id = request.POST.get('current_dpt_id', '')
        data["dept"] = get_dept_list(lists_dpt)
    elif action == "dept_member" :
        dept_id = request.POST.get('dept_id', '')
        dept_id = -1 if not dept_id else int(dept_id)
        if dept_id in (0, -1):
            for objBox in lists_mbox:
                objMember = DepartmentMember.objects.filter(mailbox_id=objBox.id).first()
                if objMember:
                    continue
                #print ">>>>>>>>> objBox ",objBox
