  ...
  <div class="j-module-content j-maillist mllist-list height100 ">
    ...
    <section class="content content-list height100" style="background-color: #fff;padding-bottom: 13px;">
      <el-form :model="sForm" label-width="180px" :rules="sFormRules" ref="sForm" size="mini" style="margin-left:13px;margin-right:13px;margin-top: 13px" :inline="true">
        ...
        <el-row><el-form-item label="保存Webmail已发送邮件"><el-radio-group v-model="sForm.save_webmail_sent"><el-radio v-for="item in disabled_options" :label="item.value">{{item.label}}</el-radio></el-radio-group></el-form-item></el-row>
        <el-row><el-form-item label="保存客户端已发送邮件"><el-radio-group v-model="sForm.save_client_sent"><el-radio v-for="item in disabled_options" :label="item.value">{{item.label}}</el-radio></el-radio-group></el-form-item></el-row>
        <el-row><el-form-item label="自动保存收件人到通讯录"><el-radio-group v-model="sForm.save_contact"><el-radio v-for="item in disabled_options" :label="item.value">{{item.label}}</el-radio></el-radio-group></el-form-item></el-row>
        <el-row><el-form-item label="邮件自动回执"><el-radio-group v-model="sForm.auto_receipt"><el-radio v-for="item in disabled_options" :label="item.value">{{item.label}}</el-radio></el-radio-group></el-form-item></el-row>
        <el-row><el-form-item label="邮件编辑时自动保存草稿"><el-radio-group v-model="sForm.auto_save_draft"><el-radio v-for="item in disabled_options" :label="item.value">{{item.label}}</el-radio></el-radio-group></el-form-item></el-row>
  ...
  export default {
    data() {
      return {
        ...
        send_charsets: [{ value: 'utf-8', label: 'UTF-8'}, {value: 'gb2312', label: 'GB2312' }],
